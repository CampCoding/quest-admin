...
const Flash_Cards = ({ CourseId, unitId, allunitdata, cd }) => {
  ...
  const getResponse = () => {
    ...
    axios
      .post(
        "https://camp-coding.tech/dr_elmatary/admin/mcq/select_unit_mcqs.php",
        JSON.stringify(data_send)
      )
      .then((res) => {
        //
        if (Array.isArray(res.message)) {
          setunitquestions(res.message);
        }
        setselectedquestion(res?.message[0]?.question_id);
      });
  };
  const getTopics = () => {
    const data_send = {
  ...
  const columns = [
    ...
    },
    {
      ...
      Cell: (cell) => {
        return <b>{cell.cell.row.flash_card_side_1}</b>;
      },
    },
    {
      ...
      Cell: (cell) => {
        return <b>{cell.cell.row.flash_card_side_2}</b>;
      },
    },
    {
      Header: "Status",
