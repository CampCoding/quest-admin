...
const AddCopoun = () => {
  document.title = "Add Copoun | Quest ";

  const checkType = window.location.pathname.includes("add-ebook-copoun");
  ...
  const getBook = async () => {
    ...
    const co = blocked.message.map((item) => {
      // console.log("Course", item);
      return {
        id: item.value,
        name: item.label,
  ...
  const addCopoun = async (e) => {
    ...
    if (allPropertiesNotEmpty) {
      const link = checkType
        ? `https://camp-coding.tech/quest/platform/admin/subscription/new_create_books_cards.php?book_ids=${data_send.subjects}&card_count=${data_send.copoun_quantity}&title=${data_send.copoun_name}&support_no=${data_send.phone_number}&end_date=${data_send.end_date}`
        : `https://camp-coding.tech/quest/platform/admin/subscription/new_create_cources_cards.php?courses_ids=${data_send.subjects}&card_count=${data_send.copoun_quantity}&title=${data_send.copoun_name}&support_no=${data_send.phone_number}&end_date=${data_send.end_date}`;

      const url = await axios.get(link);
      window.open(url?.split("king_mo_reda")[1]?.split(".xlsx")[0] + ".xlsx");
                      ...
                      >
                        ...
                        <div className="mb-3">
                          ...
                          <input
                            id="end_date"
                            name="end_date"
                            placeholder="Enter Copoun End Date"
                            type="date"
                            className="form-control"
                          />
