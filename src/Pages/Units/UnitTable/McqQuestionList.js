...
const TopicMcqQuestionList = ({
  ...
  const handelDelete = async () => {
    axios
      .post(
        "https://camp-coding.tech/quest/platform/admin/mcq/delete_mcq.php",
        { question_id: rowdata?.question_id }
      )
      .then((res) => {
        if (res.status == "success") {
                  ...
                  onClick={() => {
                    ...
                    for (
                      ...
                      let obj = {
                        ...
                        checked:
                          cell.cell.row.original?.arrAns[i].answer_score ==
                          "true"
                            ? true
                            : false,
                      };
  ...
  const passedColumns = [
    ...
    },
    {
      Header: "Topic",
      Cell: (cell) => {
        return <b>{cell.cell.row?.original?.topic_label} </b>;
      },
    },
    {
      Header: "Unit",
      Cell: (cell) => {
        return <b>{cell.cell.row?.original?.unit_details?.unit_name} </b>;
      },
    },
    {
      Header: "Course",
      Cell: (cell) => {
        return <b>{cell.cell.row?.original?.course_details?.course_name} </b>;
      },
    },
    {
      Header: "Grade",
      Cell: (cell) => {
        return <b>{cell.cell.row.original?.course_details?.grade_name} </b>;
      },
    },
    {
      Header: "University",
      Cell: (cell) => {
        return (
          <b>{cell.cell.row.original?.course_details?.university_name} </b>
        );
      },
    },
          ...
          <>
            ...
            <button
              ...
              onClick={() => {
                ...
                return setOverAllImages(
                  cell.cell.row.original.array_overall_image
                    ? cell.cell.row.original.array_overall_image?.split(
                        "**matary**"
                      )
                    : []
                );
  ...
  return (
    <React.Fragment>
      ...
      {Units && Units.length ? (
        <TableContainer
          columns={!passed ? columns : passedColumns}
          data={Units}
          isGlobalFilter={true}
          customPageSize={10}
        ...
        children={
          <form action="#" onSubmit={(e) => e.preventDefault()}>
                ...
                overAllImages?.map((item, index) => {
                  return (
                    <div className="postImage">
                      ...
                      <div
                        className="btn btn-danger"
                        onClick={() => (loading ? null : deleteImage(index))}
                      >
            ...
            {uploadLoading ? (
              ...
              <div
                className="btn btn-success"
                onClick={() => (!uploadLoading ? addAllImages() : null)}
              >
                Add Images
              </div>
            )}
            {
              <div
                className="btn btn-success"
                onClick={() => (!uploadLoading ? handleUpdateImages() : null)}
              >
                Update Images
              </div>
            }
        ...
        children={
          <form action="#" onSubmit={(e) => e.preventDefault()}>
                ...
                expImages?.map((item, index) => {
                  return (
                    <div className="postImage">
                      ...
                      <div
                        className="btn btn-danger"
                        onClick={() => (loading ? null : deleteExpImage(index))}
                      >
                        {loading ? <Loader /> : "Delete"}
                      </div>
                    </div>
            ...
            {uploadLoading ? (
              ...
              <div
                className="btn btn-success"
                onClick={() => (!uploadLoading ? handleuploadExpimg() : null)}
              >
                Add Images
              </div>
            )}
            {
              <div
                className="btn btn-success"
                onClick={() =>
                  !uploadLoading ? handleUpdateAnswerImages() : null
                }
              >
                Update Images
              </div>
      ...
      <Modal isOpen={!!deleteId} toggle={() => setDeleteId(false)} centered>
        ...
        <ModalBody>
          <h5>Are You Sure To Delete Question ?</h5>
        </ModalBody>
        <ModalFooter>
...
export default TopicMcqQuestionList;
