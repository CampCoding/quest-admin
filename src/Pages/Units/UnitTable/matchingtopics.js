...
import CopyTopic from "./MatchingQuestions/matchingTopicsManagement/copyTopic";
import EditTopic from "./MatchingQuestions/matchingTopicsManagement/editTopic";
import ShowHideTopic from "./MatchingQuestions/matchingTopicsManagement/showHideTopic";
...
const TrueFalseTopics = ({ Units, courseData, setshowconf, setrowdata }) => {
  ...
  const [MatchingQs_Show, setMatchingQsShow] = useState(false);
  ...
  const DownloadAllQuestions = async (topic_label) => {
    ...
    try {
      const Download = await axios.post(
        "https://camp-coding.tech/quest/platform/admin/excil_data/MatchingQs/mcq_data_by_topic.php",
        {
  ...
  const getTopics = async () => {
    const topic = await axios.post(
      "https://camp-coding.tech/quest/platform/admin/matching_qs/topics/select_topics.php",
      {
        course_id: unitData?.course_id,
        unit_id: unitData?.unit_id,
      }
  ...
  const uploadExcel = async (e, type) => {
    ...
    const excelUploading = await axios.post(
      type == "questions"
        ? "https://camp-coding.tech/quest/platform/admin/true_false_ques/upload_excel_file_wqs.php"
        : type == "tweets"
  ...
  return (
    <React.Fragment>
      ...
      <Modal title="Upload Excel File" isOpen={MatchingQs_Show}>
        <form
          action="#"
          style={{
            padding: "15px",
